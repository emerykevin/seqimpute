mict.internal <- function(data, imp, MaxGap, 
  ...
  available, REFORD_L, noise, verbose, ...)
{
  ...
  for (order in 1:MaxGap) {
    ...
    train <- compute.traindata(data, MaxGap, order, shift, np, nc, nr, nf, k,
                                pastDistrib, futureDistrib, col=0, 
                                frame.radius=0, regr, timing=FALSE)
    
    if (length(table(train[, 1])) > 1) {
      reglog <- fitmodel(train, regr, ...)
      
      imp <- CreatedModelImputation(order, covariates, train, 
        time.covariates, COtsample, OD, imp, pastDistrib, futureDistrib, available, 
        REFORD_L, ncot, nc, np, nf, k, regr, reglog, noise, 
        shift, MaxGap)
...
CreatedModelImputation <- function(order, CO, train, COt, COtsample, OD, ODi, pastDistrib, 
                                   ...
                                   noise, shift, MaxGap)
{
  ...
  ODi <- ODiImputePF(CO, ODi, train, COt, COtsample, REFORD, nr_REFORD, pastDistrib, 
