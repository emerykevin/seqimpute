mict.timing <- function(data, imporder, regr = "multinom", np = 1, nf = 0, nfi = 1, 
  npt = 1, available = TRUE, pastDistrib = FALSE,
  futureDistrib = FALSE, m = 1, frame.radius = 0, verbose = TRUE, ...)
{
  ...
  if(imporder$maxInitial > 0){
    ...
    imp <- mict.timing.impute(data, imp,
                            imporder$maxInitial, regr, np=0, nfi, 
  ...
  if (imporder$maxTerminal >0) {
    ...
    imp <- mict.timing.impute(data, imp,
                            imporder$maxTerminal, regr, npt, nf=0, 
    ...
    for(g in 2:np){
      for (h in (data$nc - 1):(data$nc - nf + 1)) {
        if(imporder$maxBothSLG[g,h] > 0){
          ...
          imp <- mict.timing.impute(data, imp,imporder$maxBothSLG[g,h], 
                            regr, np_temp, nf_temp, pastDistrib, futureDistrib, 
                            available, imporder$REFORDSLGBoth[[g]][[h]], 
...
mict.timing.impute <- function(data, imp, MaxGap,regr, np, nf, 
  ...
  for (order in 1:MaxGap) {
    if(verbose == TRUE){print(paste0("Step ", order, "/", MaxGap))}
    ...
    for (i in 1:length(col_to_imp)){
      ...
      train <- compute.traindata(data, MaxGap, order, shift, min(np, col - 1), nc, nr, 
                                  min(nf, nc - col), k, pastDistrib, futureDistrib,
                                  col, frame.radius, regr, timing=TRUE)

      if (length(table(train[, 1])) > 1){
        reglog <- fitmodel(train, regr, ...)
        
        imp <- impute.timing(data, order, train,
...
compute.traindata <- function(data, MaxGap, order, shift, np, nc, nr, nf, k,
  ...
  col_to_use <- compute.coluse(timing, MaxGap, np, nf, frame.radius, order, shift, nc, col)
...
compute.coluse <- function(timing, MaxGap, np, nf, frame.radius, order, shift, nc, col){
  if(timing==TRUE){
    if ((np > 0 & nf > 0) & ((MaxGap %% 2 == 0 & order %% 2 == 0) |
                             (MaxGap %% 2 != 0 & order %% 2 != 0))) {
...
fitmodel <- function(data, regr, ...) {
  ...
  if (regr == "rf") { 
    ...
    reglog <- nnet::multinom(data, maxit = 100, trace = FALSE, 
                             MaxNwts = 1500, ...)
  } else {
  }
...
impute.timing <- function(data, order, CD, imp, 
                                         pastDistrib, futureDistrib, available, col, row_to_imp, np, nf,
  ...
  if (pastDistrib) {
    ...
    CDdb[, ] <- compute.distrib(LOOKUP[row_to_imp,,drop=FALSE], nc, k, col, type="past")
  ...
  if (futureDistrib) {
    ...
    CDda[, ] <- compute.distrib(LOOKUP[row_to_imp,,drop=FALSE], nc, k, col, type="future")
  ...
  if(regr=="multinom"){
    ...
    for(u in 1:length(row_to_imp)){
      ...
      pred <- predict(reglog, data = CDi[u,], predict.all = TRUE)$predictions[1, ]
